...
function test_urlEncode(){
	var lab="李天一";
	console.log(encodeURIComponent(encodeURIComponent(lab)));
}
function test_reg(){
	var fetch=require('./fetcher').fetch;
	 fetch("http://127.0.0.1:8890/","http://weibo.com/1819750005/Adj8K1igr?type=comment",function(err,data){
					var s=data.match(/&page=(.*?)[\\]/g,'$1');
					var nexts=[];
					s.forEach(function(elem){
						elem=elem.replace(/&page=(.*?)[\\]/g,'$1');
						console.log('1\t'+elem);
						elem=elem.replace(/&page=([0-9].*)[\\].*?/g,'$1');
						console.log('2\t'+elem);
						nexts.push(elem);
					});
		//			console.log(nexts);
