...
//	var cookie=new Cookie({ip:'192.168.86.216'});
	...
	crawler.setHandler('even_f',Handlers.publicEvenF);
	crawler.setHandler('even_s',Handlers.publicEvenS);
...
//	 push_qq_user({uid:'daricyang',following:134,follower:39});
	/*-----------end tencent method---------------*/
	function push_even(key){
		var urls=[];
		var url=encodeURIComponent(encodeURIComponent(key));
		url='http://s.weibo.com/weibo/'+url+'&xsort=hot/';
		urls.push(url);
		for(var i=2;i<=50;i++)
			urls.push(url+'&page='+i+'/');
		crawler.push({urls:urls,handler:'even_f',extra:key});
	}
	push_even('李天一');
}
