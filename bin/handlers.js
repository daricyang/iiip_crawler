...
var publicEvenSaver=new MongoSaver({dbName:'public_even',collectionName:'c_even_page'});
var publicEvenSaverBatch=new Batch({
	name:'publicEvenSaver'
	,op:publicEvenSaver.save.bind(publicEvenSaver)
});
/*------------end tencent method--------*/
/*------------------------------------end batch inital----------------------------------------*/
 ...
 * */
/*-------------------public even--------------*/
exports.publicEvenF=function(data,callback,finish){
	_self=this;
	var nexts=[];
	var extract_mblog=function(done){
		var tmp=data.html.match(/&url=(.*?)&mid/g,'$1');
		if(tmp&&tmp.length!=0){
			tmp.forEach(function(elem){
				elem=elem.replace(/&url=(.*?)&mid/g,'$1');
				elem=elem.replace(/[\\]/g,'');
				nexts.push(elem+'?type=comment');
				nexts.push(elem+'?type=repost')
			});
	...
	extract_mblog(finish);	
};
exports.publicEvenS=function(data,callback,finish){
	console.log('publicEvenS\t'+data.url);
	_self=this;
	var nexts=[];
	var extract_cr=function(done){
		if(data.url.match(/comment$/)||data.url.match(/repost$/)){
			var tmp=data.html.match(/&page=(.*?)[\\]/g,'$1');
			var maxPage=1;
			if(tmp&&tmp.length!=0){
				tmp.forEach(function(elem){
					console.log(data.url);
					elem=elem.replace(/&page=(.*?)[\\]/g,'$1');
					if(elem>maxPage) maxPage=elem;
				});
				var url=data.url+'&page=';
	...
	extract_cr(function(){
		publicEvenSaverBatch.push({extra:data.extra,url:data.url,html:data.html,time:(new Date().getTime())});
		finish();
	});
};
/*------------------end public even-----------*/
